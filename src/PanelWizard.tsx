...
export const PanelWizard = ({ schema, fields, url }: Props) => {
  ...
  return (
    ...
    >
      <InfoBox title="Configure your query" url={url} severity="info" style={{ maxWidth: '500px' }}>
        <p>{`Define a data source query that return at least the following field${report.length > 1 ? 's' : ''}:`}</p>
        <div>
...
export const validateFields = (fields: Field[], schema: FieldSchema[]): Array<FieldSchema & { ok: boolean }> => {
  const seen: Field[] = [];
  return schema.map(({ type, description }) => {
    const field = fields.filter((field) => !seen.includes(field)).find((field) => field.type === type);
    if (field) {
      seen.push(field);
    }
